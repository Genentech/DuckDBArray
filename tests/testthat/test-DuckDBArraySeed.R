...
# library(testthat); library(DuckDBArray); source("setup.R"); source("test-DuckDBArraySeed.R")
...
test_that("Arith methods work as expected for a DuckDBArraySeed", {
    ...
    checkDuckDBArraySeed(+ seed, + as.array(seed))
...
test_that("Math methods work as expected for a DuckDBArraySeed", {
    ...
    checkDuckDBArraySeed(log1p(income), log1p(as.array(income)))
    ...
    checkDuckDBArraySeed(expm1(income), expm1(as.array(income)))
